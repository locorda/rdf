...
class _Expander {
  ...
  List<Object?> _expandArray(
    ...
    for (final item in array) {
      final expanded =
          _expandElement(item, context, activeProperty, fromMap: fromMap);
      if (expanded is List) {
        result.addAll(expanded);
      } else if (expanded != null) {
        result.add(expanded);
  ...
  Object? _expandObject(
    ...
    if (propertyScoped &&
        activeContext.hasPropagate &&
        !activeContext.propagate) {
    ...
    for (final entry in entries) {
      ...
      if (resolvedKey == '@nest' && key != '@nest') {
        _processProperty(key, value, context, result,
  ...
  void _processGraph(Object? value, JsonLdContext context, JsonObject result) {
    final expanded = _expandElement(value, context, '@graph', fromMap: false);
  ...
  void _processIncluded(
    ...
    final expanded = _expandElement(value, context, null, fromMap: false);
    final List<Object?> items;
    if (expanded is List) {
      items = expanded;
    } else if (expanded != null) {
  ...
  void _processProperty(
    ...
    if (termDef?.iri == '@nest' || _resolveAlias(key, context) == '@nest') {
    ...
    if (termDef?.isReverse == true) {
      if (containers.contains('@index') && value is Map<String, Object?>) {
        ...
        if (termDef!.indexMapping != null) {
          indexItems =
              _expandPropertyIndexMap(value, valueContext, key, termDef);
        } else {
          indexItems =
    ...
    if (containers.contains('@language') && value is Map<String, Object?>) {
      expandedValue = _expandLanguageMap(value, context, termDef);
      ...
      expandedValue =
          _expandPropertyIndexMap(value, valueContext, key, termDef!);
    } else if (containers.contains('@id') && value is Map<String, Object?>) {
      expandedValue =
          _expandIdMap(value, valueContext, context, termDef: termDef);
    } else if (containers.contains('@type') && value is Map<String, Object?>) {
      expandedValue =
          _expandTypeMap(value, valueContext, context, termDef: termDef);
    } else {
    ...
    if (containers.contains('@list')) {
      ...
      final isSingleListObject =
          expandedValue is Map && expandedValue.containsKey('@list');
        ...
        if (item is Map<String, Object?>) {
          ...
          if (resolvedIndex != '@none') {
            ...
            if (indexTermDef?.typeMapping == '@vocab' ||
              ...
              final resolvedIdx = _isUsableIri(expandedIdx) ||
                      expandedIdx.startsWith('_:')
                  ? expandedIdx
                  : (_expandIriReference(expandedIdx, context) ?? expandedIdx);
  ...
  List<Object?> _expandTypeMap(
    ...
    for (final mapKey in sortedKeys) {
      ...
      if (typeTermDef != null && typeTermDef.hasLocalContext) {
        final baseForType = mapContext.nonPropagatedParent ?? mapContext;
      ...
      for (final rawItem in rawItems) {
        ...
        if (resolvedMapKey != '@none') {
          if (termDef != null && termDef.indexMapping != null) {
            ...
            if (indexTermDef?.typeMapping == '@vocab' ||
                indexTermDef?.typeMapping == '@id') {
              final expandedIdx = contextProcessor.expandIri(mapKey, context);
              final resolvedIdx = _isUsableIri(expandedIdx) ||
                      expandedIdx.startsWith('_:')
                  ? expandedIdx
                  : (_expandIriReference(expandedIdx, context) ?? expandedIdx);
  ...
  Object _expandPropertyValue(
    ...
    if (value is List) {
      ...
      if (containers.contains('@list')) {
        final items = _expandListContainerArray(value, context, activeKey);
        _checkNoListOfLists(items);
        return items;
      }
  ...
  List<Object?> _expandListContainerArray(
    ...
    for (final item in value) {
      if (item is List) {
        ...
        final expanded =
            _expandElement(item, context, activeKey, fromMap: false);
        if (expanded is List) {
          result.addAll(expanded);
        } else if (expanded != null) {
          result.add(expanded);
  ...
  Object? _finalizeObject(
    ...
    final resolvedActiveProperty = activeProperty != null
        ? _resolveAlias(activeProperty, activeContext)
        : null;
  ...
  Object? _finalizeValueObject(JsonObject result, JsonLdContext context) {
    ...
    final invalidKey = result.keys
        .firstWhere((k) => !allowedKeys.contains(k), orElse: () => '');
