...
void main() {
  ...
  group('W3C JSON-LD fromRdf Test Suite', () {
    ...
    final skippedTests = testCases.where((t) => t.specVersion == 'json-ld-1.0');
    for (final tc in skippedTests) {
      test(tc.name, skip: 'Requires json-ld-1.0 semantics', () {});
    }

    final supported = testCases.where((t) => t.specVersion != 'json-ld-1.0');
    final positiveTests =
    ...
    group('Positive eval tests (${positiveTests.length})', () {
      for (final tc in positiveTests) {
        test(tc.name, () {
          final inputNq = File('$testsDir/${tc.input}').readAsStringSync();
          final expectedJson =
