...
/// Automated release script for rdf_canonicalization
///
...
void main(List<String> args) async {
  ...
  print('rdf_canonicalization release script');
  print('------------------------');
  if (dryRun) {
...
void _printUsage() {
  ...
  print('-------------');
  print('Automates the release process for the rdf_canonicalization package.');
  print('');
  print('Usage:');
...
Future<void> _ensureNoDevVersionStrings(String releaseVersion) async {
  ...
  for (final filePath in filesToCheck) {
    ...
    final wrongPubAddPattern =
        RegExp(r'dart pub add rdf_canonicalization:[^\s"]+');
    ...
    if (devPattern.hasMatch(content) || wrongPubAddPattern.hasMatch(content)) {
      ...
      content = content.replaceAll(
          wrongPubAddPattern, 'dart pub add rdf_canonicalization');
