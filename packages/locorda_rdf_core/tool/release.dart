...
/// Automated release script for locorda_rdf_core
///
...
void main(List<String> args) async {
  ...
  print('locorda_rdf_core release script');
  print('------------------------');
  if (dryRun) {
...
void _printUsage() {
  ...
  print('-------------');
  print('Automates the release process for the locorda_rdf_core package.');
  print('');
  print('Usage:');
...
Future<void> _ensureNoDevVersionStrings(String releaseVersion) async {
  ...
  for (final filePath in filesToCheck) {
    ...
    final wrongPubAddPattern = RegExp(r'dart pub add locorda_rdf_core:[^\s"]+');
    ...
    if (devPattern.hasMatch(content) || wrongPubAddPattern.hasMatch(content)) {
      ...
      content = content.replaceAll(
          wrongPubAddPattern, 'dart pub add locorda_rdf_core');
