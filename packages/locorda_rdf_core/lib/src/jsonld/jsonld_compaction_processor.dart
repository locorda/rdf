...
class JsonLdCompactionProcessor {
  ...
  Object? _compact({
    ...
    if (expandedMap.containsKey('@value')) {
      ...
      if (compactedValue is! Map<String, Object?> ||
          !compactedValue.keys.any((k) =>
              k.startsWith('@') ||
              _isKeywordAlias(activeContext, k))) {
    ...
    if (expandedMap.containsKey('@type')) {
      ...
      if (!identical(typeScopedContext, activeContext)) {
        ...
        if (typeScopedContext.hasPropagate &&
            typeScopedContext.propagate) {
    ...
    for (final expandedProperty in keys) {
      ...
      if (expandedProperty == '@type') {
        ...
        final typeTermDef =
            _lookupTermByCompactIri(typeScopedContext, alias);
        ...
        final forceArray = typeHasSetContainer &&
          ...
          for (final entry in compactedReverse.entries) {
            ...
            if (termDef?.isReverse == true) {
              _addValue(result, entry.key, entry.value,
                  asArray:
                      _hasSetContainer(typeScopedContext, entry.key));
      ...
      if (expandedProperty == '@preserve') {
        ...
        if (compactedPreserve is! List ||
            (compactedPreserve).isNotEmpty) {
      ...
      if (expandedValue is List && expandedValue.isEmpty) {
        ...
        _addValue(nestResult, compactedProp, <Object?>[],
            asArray: true);
        continue;
      }
      ...
      final valueList =
          expandedValue is List ? expandedValue : [expandedValue];
      ...
      for (final expandedItem in valueList) {
        ...
        if (isList) {
          ...
          if (hasTypeScope &&
              ...
              !(expandedItem.length == 1 &&
                  expandedItem.containsKey('@id'))) {
        ...
        if (container.contains('@language') &&
          ...
          if (directionOk) {
            final langKey =
                (expandedItem['@language'] as String?) ?? '@none';
            final mapValue =
                compactedItem is Map<String, Object?> &&
        ...
        if (container.contains('@index') &&
          ...
          if (indexProperty != null && indexProperty != '@index') {
            ...
            mapKey = _extractPropertyIndex(
                compactedItem, compactedIndexProp) ?? '@none';
          } else {
        ...
        _addValue(nestResult, compactedProp, compactedItem,
            asArray: asArray);
      }
    }
  ...
  Object? _compactGraphContainer({
    ...
    final graphArray =
        graphContent is List ? graphContent : [graphContent];
    final nestResult = _getNestTarget(
        result, typeScopedContext, typeScopedInverse, compactedProp);
    ...
    if (container.contains('@id')) {
      ...
      if (graphArray.length == 1) {
        graphValue = compactArrays && !hasSetContainer
            ? graphArray[0]
            : graphArray;
      } else {
    ...
    if (value is Map<String, Object?>) {
      ...
      if (value is Map<String, Object?> && value.containsKey('@list')) {
        ...
        if (listValue is List && listValue.isEmpty) {
          ...
          final effectiveLang =
              (allSameLanguage && commonLanguage != null)
                  ? commonLanguage
                  : '@none';
        ...
        for (final prefVal in preferredValues) {
          ...
          if (anyTerm != null) {
            ...
            if (anyDef != null &&
              ...
              if (anyDef.hasDirection &&
                  anyDef.direction != null) {
  ...
  String _compactIriRelativeToBase(JsonLdContext context, String iri) {
    ...
    if (base != null) {
      try {
        ...
        final relative = relativizeIri(iri, base,
            options: _jsonLdRelativizeOptions);
        if (relative != iri) {
    ...
    for (final term in sortedTerms) {
      ...
      if (hasLanguage && hasDirection) {
        ...
        containerMap
            .putIfAbsent('@language', () => <String, String>{})
            .putIfAbsent(
                _activeContextLanguageKey(context), () => term);
        containerMap
            .putIfAbsent('@type', () => <String, String>{})
      ...
      if (!def.isReverse) {
        final isKeywordType = normalizedType == '@id' ||
            ...
            normalizedType == '@none' ||
            normalizedType == '@json';
        final hasSpecificConstraint =
            (typeMapping != null && !isKeywordType) ||
  ...
  String? _extractPropertyIndex(
      Object? compactedItem, String indexProperty) {
    if (compactedItem is Map<String, Object?>) {
