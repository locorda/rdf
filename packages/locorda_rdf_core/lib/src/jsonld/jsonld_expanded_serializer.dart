...
final class _GraphSerializer {
  ...
  List<Map<String, Object?>> serialize(RdfDataset dataset) {
    ...
    for (final ng in dataset.namedGraphs) {
      ...
      final graphNode =
          defaultNodes.putIfAbsent(graphId, () => <String, Object?>{'@id': graphId});
  ...
  void _serializeGraph(
    ...
    for (final triple in deduped) {
      ...
      final node =
          nodeMap.putIfAbsent(subjectId, () => <String, Object?>{'@id': subjectId});
    ...
    for (final entry in subjectPredicateCounts.entries) {
      ...
      if (extraPreds.isEmpty) {
        ...
        final typeTriples = triples.where((t) =>
            t.subject == bn &&
            (t.predicate as IriTerm).value == _rdfType);
        final allList = typeTriples.every(
            (t) => t.object is IriTerm && (t.object as IriTerm).value == _rdfList);
  ...
  Map<String, Object?> _convertLiteral(LiteralTerm literal) {
    ...
    if (rdfDirection == 'i18n-datatype' &&
        datatypeIri.startsWith(_i18nBase)) {
