...
class TriGParser {
  ...
  TriGParser(String input,
      {String? baseUri,
      Set<TriGParsingFlag> parsingFlags = const {},
      RdfNamespaceMappings namespaceMappings = const RdfNamespaceMappings(),
      IriTermFactory iriTermFactory = IriTerm.validated,
      required String format})
      : _tokenizer = TriGTokenizer(input, parsingFlags: parsingFlags),
        _baseUri = baseUri,
  ...
  List<Quad> parse() {
    try {
      ...
      _log.fine('Parse complete. Found ${quads.length} triples');
      return [...quads, ..._quads];
    } catch (e, stack) {
